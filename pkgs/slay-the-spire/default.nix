{ lib, ... }:
/*
  ?? Missing Config file, trys to write a default one to readonly area ??
*/
lib.builders.mkGogPackage rec {
  ...
  fixup = ''
    sed -i "s/jre\/bin\/java/SlayTheSpire/" $out/share/${pname}-${version}/data/noarch/start.sh
  '';
}
